...
import { ContractProjectData, Signatory } from '../../types';
import { calculateProjectSummary, formatCurrency } from '../../utils/calculations';
import { resetPage4Data } from '../../utils/pageResetHelpers';
...
interface Page4MonthlyMemoProps {
  project: ContractProjectData;
  onChange: (updatedProject: ContractProjectData) => void;
}

export const Page4MonthlyMemo: React.FC<Page4MonthlyMemoProps> = ({
  project,
  onChange,
}) => {
  ...
  const handleSignatoryChange = (key: keyof ContractProjectData['signatories'], subField: keyof Signatory, value: string) => {
    onChange({
      ...project,
      signatories: {
        ...project.signatories,
        [key]: {
          ...project.signatories[key],
          [subField]: value,
        },
      },
    });
  ...
  const handleClearPage4 = () => {
    onChange(resetPage4Data(project));
  };

  return (
    <div id="page-4-monthly-memo" className="space-y-6">
      {/* Page Title with Inset Pill Capsule */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <ClearPageButton pageNumber={4} onClear={handleClearPage4} />
      </div>
      ...
      <div className="neu-flat p-6 sm:p-8 rounded-3xl space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label htmlFor="field-monthly-selectedMonth" className="block text-xs font-bold text-slate-400 mb-1.5">
              รายงานประจำเดือน:
            </label>
            <input
              id="field-monthly-selectedMonth"
              type="text"
              value={project.selectedMonth}
              onChange={(e) => handleFieldChange('selectedMonth', e.target.value)}
              placeholder="เช่น สิงหาคม 2567"
              className="w-full neu-input px-4 py-2.5 rounded-2xl text-sm font-semibold text-orange-400"
            />
          </div>

            <div>
              <label htmlFor="field-monthly-periodStart" className="block text-xs font-bold text-slate-400 mb-1.5">
                ตั้งแต่วันที่:
              </label>
              <input
                id="field-monthly-periodStart"
                type="date"
            ...
            <div>
              <label htmlFor="field-monthly-periodEnd" className="block text-xs font-bold text-slate-400 mb-1.5">
                ถึงวันที่:
              </label>
              <input
                id="field-monthly-periodEnd"
                type="date"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="field-monthly-projectName" className="block text-xs font-bold text-slate-400 mb-1.5">
                โครงการ:
              </label>
              <input
                id="field-monthly-projectName"
                type="text"
            ...
            <div>
              <label htmlFor="field-monthly-location" className="block text-xs font-bold text-slate-400 mb-1.5">
                สถานที่ก่อสร้าง:
              </label>
              <input
                id="field-monthly-location"
                type="text"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label htmlFor="field-monthly-contractNumber" className="block text-xs font-bold text-slate-400 mb-1.5">
                สัญญาจ้างเลขที่:
              </label>
              <input
                id="field-monthly-contractNumber"
                type="text"
            ...
            <div>
              <label htmlFor="field-monthly-startDate" className="block text-xs font-bold text-slate-400 mb-1.5">
                วันที่เริ่มต้นสัญญา:
              </label>
              <input
                id="field-monthly-startDate"
                type="date"
            ...
            <div>
              <label htmlFor="field-monthly-endDate" className="block text-xs font-bold text-slate-400 mb-1.5">
                วันที่สิ้นสุดสัญญา:
              </label>
              <input
                id="field-monthly-endDate"
                type="date"
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          <div className="neu-section-capsule">
            ...
            <h3 className="text-sm sm:text-base font-bold text-slate-100">
              ตารางสรุปผลการดำเนินงานประจำเดือน
            </h3>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
          ...
          <div className="neu-flat p-4 rounded-3xl space-y-1.5" id="stat-monthly-progress">
            ...
            <span className="text-[11px] text-slate-400 block">จากตารางสะสมรายเดือน</span>
          </div>

          {/* 2: Ahead / Behind Plan */}
          <div className="neu-flat p-4 rounded-3xl space-y-1.5" id="stat-monthly-variance">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
          ...
          {/* 3: Remaining Days */}
          <div className="neu-flat p-4 rounded-3xl space-y-1.5" id="stat-monthly-days">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
          ...
          {/* 4: Disbursed Amount */}
          <div className="neu-flat p-4 rounded-3xl space-y-1.5" id="stat-monthly-disbursed">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">
              วงเงินเบิกจ่ายแล้ว
            </span>
      ...
      <div className="neu-flat p-6 rounded-3xl space-y-4">
        <div className="neu-section-capsule">
          ...
          <h3 className="text-sm sm:text-base font-bold text-slate-100">
            ผู้ลงนามรับรองรายงานประจำเดือน
          </h3>
        </div>
